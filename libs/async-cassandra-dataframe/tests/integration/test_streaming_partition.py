...
class TestStreamingPartition:
    ...
    async def test_split_token_ring(self, session):
        ...
        # Test various split counts
        for num_splits in [1, 2, 4, 10, 100]:
            ranges = strategy._split_token_ring(num_splits)

            # Should have correct number of ranges
            assert len(ranges) == num_splits

            # First range should start at MIN_TOKEN
            assert ranges[0][0] == strategy.MIN_TOKEN

            # Last range should end at MAX_TOKEN
            assert ranges[-1][1] == strategy.MAX_TOKEN

            # No gaps or overlaps
            for i in range(1, len(ranges)):
                # End of previous + 1 should equal start of current
                assert ranges[i - 1][1] + 1 == ranges[i][0]
    ...
    async def test_create_fixed_partitions(self, session, basic_test_table):
        ...
        partitions = await strategy.create_partitions(
            basic_test_table, ["id", "name", "value"], partition_count=5  # Fixed count
        )

        # Should have exactly 5 partitions
        assert len(partitions) == 5

        # Check partition structure
        for i, partition in enumerate(partitions):
            assert partition["partition_id"] == i
            assert partition["table"] == basic_test_table
            assert partition["columns"] == ["id", "name", "value"]
            assert partition["strategy"] == "fixed"
    ...
    async def test_create_adaptive_partitions(self, session, basic_test_table):
        ...
        partitions = await strategy.create_partitions(
            basic_test_table, ["id", "name", "value"], partition_count=None  # Adaptive
        )
        ...
        for partition in partitions:
            assert partition["strategy"] == "adaptive"
            assert partition["memory_limit_mb"] == 50
            assert "estimated_rows" in partition
            assert "avg_row_size" in partition
            assert partition["avg_row_size"] > 0
    ...
    async def test_stream_partition_memory_limit(self, session, test_table_name):
        ...
        try:
            ...
            # Should have read some rows but not all
            assert len(df) > 0
            assert len(df) < 100  # Didn't read all due to memory limit
    ...
    async def test_stream_partition_token_range(self, session, basic_test_table):
        ...
        ranges = strategy._split_token_ring(4)

        # Read first range only
        partition_def = {
            "table": basic_test_table,
            "columns": ["id", "name"],
            "start_token": ranges[0][0],
            "end_token": ranges[0][1],
            "memory_limit_mb": 128,
            "primary_key_columns": ["id"],
        }
