...
class TestUDTComprehensive:
    ...
    async def test_basic_udt(self, session, test_table_name):
        ...
        try:
            ...
            # Try to handle string representation
            ...
            # Handle string representation for work address too
    ...
    async def test_collections_of_udts(self, session, test_table_name):
        ...
        try:
            ...
            # Verify list order preserved
            assert phone_list[0]["type"] == "mobile"
            assert phone_list[1]["type"] == "home"
            assert phone_list[2]["type"] == "work"

            # Verify UDT fields
            assert phone_list[0]["number"] == "555-0001"
            assert phone_list[0]["country_code"] == 1
            ...
            assert isinstance(phone_set, list | set)  # May be converted to list
            assert len(phone_set) == 2

            # Convert to set for comparison
            phone_types = {p["type"] for p in phone_set}
            ...
            assert isinstance(phone_map, dict)
            assert len(phone_map) == 2
            assert "primary" in phone_map
            assert "secondary" in phone_map

            assert phone_map["primary"]["type"] == "mobile"
            assert phone_map["primary"]["number"] == "555-0001"
            assert phone_map["secondary"]["type"] == "home"
    ...
    async def test_frozen_udt_in_primary_key(self, session, test_table_name):
        ...
        try:
            ...
            unique_tenants = (
                pdf["tenant"]
                .apply(lambda x: (x["organization"], x["department"], x["team"]))
                .unique()
            )
            ...
            assert isinstance(first_tenant, dict)
            assert "organization" in first_tenant
            assert "department" in first_tenant
            assert "team" in first_tenant
    ...
    async def test_udt_with_all_types(self, session, test_table_name):
        ...
        try:
            # Prepare test values
            test_uuid = uuid4()
            test_timeuuid = uuid4()  # Would use uuid1() for time-based
            ...
            # Temporal types
            assert isinstance(data["date_field"], date)
            assert data["date_field"] == date(2024, 1, 15)

            # Other types
            assert data["boolean_field"] is True
            ...
            assert data2["boolean_field"] is False
    ...
    async def test_udt_writetime_ttl(self, session, test_table_name):
        ...
        try:
            ...
            df = await cdf.read_cassandra_table(
                f"test_dataframe.{test_table_name}", session=session, writetime_columns=["name"]
            )
            ...
            assert abs((name_writetime - base_time).total_seconds()) < 1
