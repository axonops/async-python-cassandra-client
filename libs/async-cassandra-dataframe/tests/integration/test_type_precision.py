...
class TestTypePrecision:
    ...
    async def test_integer_types_precision(self, session, test_table_name):
        ...
        try:
            ...
            # Varint should be object to preserve unlimited precision
            assert (
                pdf["varint_col"].dtype == "object"
    ...
    async def test_decimal_and_float_precision(self, session, test_table_name):
        ...
        try:
            ...
            # Float/Double types
            assert pdf["float_col"].dtype == "float32"
            assert pdf["double_col"].dtype == "float64"

            # Special values
            assert np.isnan(pdf.iloc[3]["float_col"])
            assert np.isinf(pdf.iloc[3]["double_col"])
            assert np.isneginf(pdf.iloc[4]["float_col"])
